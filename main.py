...
class App:
    def __init__(self, master):
        ...
        self.contact_button.pack(anchor='n')
        ...
        self.excel_button = tk.Button(self.master, text="Create an Excel file", font=(text_style),bg=color_btn, command=self.create_excel)
    ...
    def submit(self):
       ...
       # get the path for dir to import the json 
        ...
        # filter the list cv with  specific language 
        choosed_resume_lang=[]
        ...
        self.create_pdf_button.pack(side=tk.LEFT, padx=(10,0))
        self.dashboard_button.pack(side=tk.LEFT, padx=(20,0))
        self.excel_button.pack(side=tk.LEFT, padx=(30,))
    ...
    def upload_job_requirements_file(self):
        ...
        if file_path:
            ...
            #A warehouse of words that recruits use in posting job requirements, and which are not keywords
            non_keywords_list =['advantage', 'mind', 'role', 'have', 'both','can',
                                'skills', 'knowledge', 'work', 'problem',
    ...
    def create_pdf(self):
        try:
            ...
            for resume in filtered_resumes:
                ...
                pdf.set_font('Arial', 'B', 10) # set bold font
                pdf.cell(30, 5, 'Phone: ', 0) # print bold text
                pdf.set_font('Arial', '', 10) # set regular font
                pdf.cell(30, 5, str(resume['phone']), 0) # print regular text
                pdf.cell(0, 5,  ' ', ln=1)  # adds a new line after this line
            ...
            if self.user_language.get() =="Hebrew":
                    self.resu_filter_label.config(text=f"נוצר בהצלחה filtered_Matched_resumes.pdf  הקובץ", fg="green")
            else:
                    self.resu_filter_label.config(text=f"File filtered_Matched_resumes.pdf was successfully created", fg="green")

        
        except Exception as e:
            # Display error message
            if self.user_language.get() == "Hebrew":
                self.resu_filter_label.config(text=f"שגיאה ביצירת הקובץ: {str(e)}", fg="red")
            else:
                self.resu_filter_label.config(text=f"Error creating file: {str(e)}", fg="red")
    ...
    def open_website(self):
        try:
            if os.path.exists("data_dashboard.html"):
                webbrowser.open("data_dashboard.html")
            else:
                messagebox.showerror("Error"," HTML page missing in yours files")
        except Exception as e:
    ...
    def create_excel(self):
        ...
        try:
            ...
            for resume in filtered_resumes:
                data['File Name'].append(resume['pdf_filename'])
                data['Matching Percentages'].append('{:.2f}%'.format(resume['percentage']))
                data['Score'].append(resume['score'])
                data['full name'].append(resume['fullname'])
                data['Email'].append(resume['email'])
                data['Phone'].append(resume['phone'])
                data['Process Start Date'].append(date.today().strftime("%d/%m/%Y"))  # Add current date
                data['Recruitment status'].append("Filtered by the matching system")

            # Create a DataFrame from the data dictionary
            df = pd.DataFrame(data)

                # Save the DataFrame as an Excel file
            df.to_excel('filtered_resumes.xlsx',index=False)
                        # Display success message
            if self.user_language.get() =="Hebrew":
                    self.resu_filter_label.config(text=f"נוצר בהצלחה filtered_resumes.xlsx  הקובץ", fg="green")
            else:
                    self.resu_filter_label.config(text=f"File filtered_resumes.xlsx was successfully created", fg="green")
    ...
    def GUI_language(self):
        ...
        if self.user_language.get() =="English":
            ...
            self.excel_button.config( text="Create an Excel file")
            ...
            self.label_lang.config(text="בחר את השפה שבה תרצה לסנן את קבצי הקורות חיים") 
            ...
            self.label_threshold.config( text="הזן את סף ההתאמה המינימלי שאתה רוצה באחוזים (0-100)") 
            ...
            self.excel_button.config(text="צור קובץ אקסל")
